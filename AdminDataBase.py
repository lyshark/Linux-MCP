...
class AdminDataBase(object):
    ...
    def AddHostGroupOnUUID(self, group_name, uuid):
        ...
        try:
            ...
            conn.commit()
            return {"code":1, "msg":f"向主机组 {group_name} 增加UUID {uuid} 完成", "data":{"group_name":group_name, "uuid":uuid}}
        except sqlite3.IntegrityError:
    ...
    def DeleteHostGroupOnUUID(self, group_name, uuid):
        ...
        conn.commit()
        conn.close()
        return {"code":1, "msg":f"从主机组 {group_name} 移除UUID {uuid} 完成", "data":{"group_name":group_name, "uuid":uuid}}
    ...
    def PingGroup(self, group_name):
        conn, cursor = self._get_conn_cursor()
        cursor.execute('''SELECT h.address, h.username FROM hosts h JOIN group_members gm ON h.uuid = gm.uuid WHERE gm.group_name=?''',
                       (group_name,))
        hosts = cursor.fetchall()
        if not hosts:
            conn.close()
            return {"code":0, "msg":f"主机组 {group_name} 不存在或无主机", "data":{}}
        ping_data = [{"address":h[0], "username":h[1], "status":"ok"} for h in hosts]
        conn.close()
        return {"code":1, "msg":f"组 {group_name} Ping测试完成", "data": ping_data}
    ...
    def ShowSingleGroup(self, group_name):
        ...
        host_list = [{"uuid":h[0],"address":h[1],"username":h[2],"password":h[3],"port":h[4]} for h in hosts]
        conn.close()
        return {"code":1, "msg":f"查询到组 {group_name} 详情，共 {len(host_list)} 台主机", "data":{"group_name":group_name, "hosts":host_list}}
    ...
    def RenameHostGroup(self, old_name, new_name):
        ...
        try:
            ...
            conn.commit()
            return {"code":1, "msg":f"主机组 {old_name} 已重命名为 {new_name}", "data":{"old_name":old_name, "new_name":new_name}}
        except Exception as e:
    ...
    def BatchDelHostFromGroup(self, group_name, uuid_list):
        ...
        return {
            ...
            "data":{"group_name":group_name, "success_count":success_count, "fail_count":fail_count, "fail_msg":fail_msg}
        }
    ...
    def ClearGroupHosts(self, group_name):
        ...
        conn.commit()
        conn.close()
        return {"code":1, "msg":f"主机组 {group_name} 内所有主机已清空（组本身保留）", "data":{"group_name":group_name}}
        ...
        try:
            with open(backup_path, 'w', encoding='utf-8') as f:
                for line in conn.iterdump():
                    f.write(line + '\n')
            return {"code":1, "msg":f"数据库备份成功", "data":{"backup_path":backup_path}}
        except Exception as e:
    ...
    def ExportHosts2CSV(self, csv_path="hosts_export.csv"):
        ...
        try:
            ...
            return {"code":1, "msg":f"所有主机数据已导出到CSV", "data":{"csv_path":csv_path, "host_count":len(hosts)}}
        except Exception as e:
    ...
    def PingAllHosts(self):
        conn, cursor = self._get_conn_cursor()
        cursor.execute("SELECT address, username FROM hosts")
        hosts = cursor.fetchall()
        conn.close()
        if not hosts:
            return {"code":0, "msg":"暂无主机数据", "data":{}}
        ping_data = [{"address":h[0], "username":h[1], "status":"ok"} for h in hosts]
        return {"code":1, "msg":"全局所有主机Ping测试完成", "data":ping_data}
